...
MAINTAINER="-g"
PREFIX="/usr/local"
XVR2LIBS=" -lxvr2 -lxvr2gl -lxvr2math "
...
echo $OSVER
echo "CFLAGS=-Wall \$(DEBUG) \$(OPTIMIZE) -DSOLARIS=1 -DSOLARIS2$OSVER=1 -I. -c -DUSE_NEW=1 ${XVR2CFLAGS} -D_STRUCTURED_PROC $DEFINES" >> Makefile
...
#DEBUG=
OPTIMIZE=
CFLAGS=-Wall \$(DEBUG) \$(OPTIMIZE) -I. ${XVR2CFLAGS} -c $DEFINES -Wimplicit -Wreturn-type -Wunused -Wswitch -Wcomment -Wparentheses -Wpointer-arith" >> Makefile
echo "LIBS=$XVR2LIBS -lm" >> Makefile
echo "INSTALLDIR=$PREFIX
LIBNAME=${MYLIBNAME}.so.${MYVERSION}
SONAME=${MYLIBNAME}.so
