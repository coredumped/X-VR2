...
class String:public System{
	private:
		...
		char *buffer;
		/**
		 * Holds the dimensions of the string plus one, the reserved address is
		 * for storing the termination character <b>0x0</b>
		 */
		int len;
		/**
		 * Eliminates the contents of buffer and sets len to
		 * zero
		 */
		void destroy();
	protected:
	public:
		/**
		 * Default constructor, it initializes buffer and len
		 * to zero 
		 */
		String();
		/**
		...
		/**
		 * transforms and concatenates s to the current 
		 * String
		 */
